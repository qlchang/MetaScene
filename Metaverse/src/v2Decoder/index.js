...
if ("function" === typeof importScripts) {
  ...
  const printConsole = {
    log: (msg) =>
      self.postMessage({ t: MessageEvent.ConsoleLog, printMsg: msg }),
    error: (msg, code) =>
      self.postMessage({
        t: MessageEvent.ConsoleError,
        printMsg: msg,
        code: code,
      }),
  };
  ...
  Decoder.prototype.checkPktOrderInConsumer = function (index) {
    ...
    if (
      this.consumerWaitingIDR ||
      this.seqDiff(this.consumerCurrPts, this.consumerPrevPts, 65536) != 1
    ) {
      // if (!mediaArray[index].isIDR && mediaArray[index].media.byteLength!=0) {
      if (
        this.isPFrame(
          mediaArray[index].isIDR,
          mediaArray[index].media.byteLength
        )
      ) {
        console.error(
          "[INFO][XMEDIA] optimize to further reduce clutter chance. copy console log to developer"
        );
        this.dumpLogBuf();
  ...
  Decoder.prototype.slotHasMedia = function (index) {
    return (
      mediaArray[index].media != null && mediaArray[index].media.byteLength != 0
    );
  };
  ...
  Decoder.prototype.startDecoding = function () {
    function iterative_getFrameToDecode() {
      ...
      self.decoder.dumpJitterBufInfo(
        "Entering Decoder.prototype.iterative_getFrameToDecode"
      );
      ...
      let expect_interval =
        ...
        (30 +
          Math.max(
            self.decoder.getNumOfPktToBeDec() -
              self.decoder.passiveJitterLength,
            0
          ));
      //let expect_interval = 1000 / (Decoder.prototype.getNumOfPktToBeDec() + 30)
      if (hasDecodeFrame) {
        let usedTime = end_ts - start_ts;
        self.decoder.getFrameInterval =
          expect_interval - Math.max(usedTime, self.decoder.dtpf);
      ...
      self.decoder.dumpJitterBufInfo(
        "Leaving Decoder.prototype.iterative_getFrameToDecode"
      );
      // refactor end
    }
    function postStats() {
      ...
      const dtpf =
        self.decoder.decodeTimeCircular.reduce(add, 0) /
          self.decoder.decodeTimeCircular.reduce(count_valid, 0) || 0;
  ...
  Decoder.prototype.initAll = function (config) {
    if (typeof wasmSource != "undefined") {
      ...
      printConsole.log(
        "wasm variable is not defined. Probably libffmpeg.js file is not loaded properly."
      );
    }
    ...
    if (ret0 == 0) {
      ...
      printConsole.error(
        "openDecoder failed with error " + String(ret0),
        "5001"
      );
      return 1;
    }
  ...
  Decoder.prototype.cacheFrame = function (data) {
    if (data.position != undefined) {
      var media = data.data.subarray(
        data.metaLen,
        data.metaLen + data.mediaLen
      );
  ...
  Decoder.prototype.isInvalidPkt = function (isIDR, mediaLen, metaLen) {
    return (
      !this.isIFrame(isIDR, mediaLen) &&
      !this.isPFrame(isIDR, mediaLen) &&
      !this.isPureMeta(metaLen, mediaLen)
    );
  };
  ...
  Decoder.prototype.notEnoughSlots = function (pts) {
    return (
      this.isBufFull() ||
      this.seqDiff(pts, this.prevSeq, CACHE_BUF_LENGTH) >
        this.getNumOfEmptySlot()
    );
  };
  ...
  Decoder.prototype.receiveFrame = function (data) {
    ...
    if (!this.aheadof(pts, this.prevSeq)) {
      ...
      if (this.seqAdd(index, 1, CACHE_BUF_LENGTH) != this.writePtr) {
        this.dumpJitterBufInfo(
          "dec worker internal info: index (" +
            index +
            ") != write_ptr (" +
            this.writePtr +
            ")"
  ...
  Decoder.prototype.decodePanorama = function (data) {
    ...
    if (ret != 0) {
      printConsole.log(
        "Decode Data error for panorama, ret value is " +
          String(ret) +
          ", frame content size: " +
          String(content_size)
      );
  ...
  */
  ...
  Decoder.prototype.decodeFrame = function (data) {
    ...
    if (content_size != 0) {
      ...
      if (ret != 0) {
        ...
        printConsole.log(
          "current poc is " + String(poc) + ", last poc is " + String(lastPoc)
        );
        return;
      }
      lastPoc = poc;
      ...
      self.decoder.dtcPtr =
        (self.decoder.dtcPtr + 1) % self.decoder.decodeTimeCircular.length;
  ...
  Decoder.prototype.LoadWASM = function (url) {
    printConsole.error("Load WASM from " + String(url));
    try {
