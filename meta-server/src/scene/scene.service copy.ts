...
export class SceneService implements OnModuleInit, OnModuleDestroy {
  ...
  async handlejoystick(request: JoystickRequest) {
    try {
      ...
      this._rotateCount = 0;
      const joystickRes: StreamFrameType =
        await this.moveService.seqExeJoystick(request);
      this.logger.log(
        'joystick-breakPointId:' +
          this.moveService.users[this.user_id].breakPointId,
      );
      // 有数据 [0]是rotate数据，[1-infinity]是walking数据
      this.logger.log('joystickRes-1', JSON.stringify(joystickRes));
      ...
      if (joystickRes?.mediaSrc) {
        ...
        this.streamService.pushFrameToSteam(streamData);

        // // 处理第一个镜头数据
        // const rotateCamData = joystickRes[0];
        // this.logger.log('镜头数据', rotateCamData.length);
        // if (rotateCamData?.length) {
        //   // 头数组[0] rotate 序列， 头是关键key
        ...
        //       joystickRes.length +
        //       ' 镜头帧数：' +
        //       joystickRes[0].length +
        //       ' 行走段数：' +
        //       (joystickRes[0]?.length
        //         ? joystickRes.length - 1
        //         : joystickRes.length) +
        //       ' 队列总帧数：' +
        //       seqs.length,
        //   );
